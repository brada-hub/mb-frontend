...
export default function ConfirmModal({ 
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className="relative w-full max-w-md bg-surface-card border border-white/10 rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                
                <div className={clsx(
                    "p-8 text-center space-y-6",
                    variant === 'danger' ? "bg-red-500/5" : "bg-brand-primary/5"
                )}>
                    <div className={clsx(
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-4">
                        <Button 
                            variant="secondary" 
                            onClick={onClose}
                            className="h-14 rounded-2xl font-bold border-white/5 active:scale-95"
                            disabled={loading}
                        >
                            {cancelText}
                        </Button>

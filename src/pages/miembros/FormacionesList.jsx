...
export default function FormacionesList() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            notify(`Error al cargar ${activeTab === 'convocatorias' ? 'convocatorias' : 'plantillas'}`, 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDeletePlantilla = async (id) => {
        ...
        try {
            ...
            notify('Error al eliminar plantilla', 'error');
        }
    };

    const filteredPlantillas = plantillas.filter(f => 
                ...
                <div>
                    ...
                    <p className="text-gray-500 text-xs font-medium uppercase tracking-widest mt-1 transition-colors">
                        {activeTab === 'convocatorias' 
                            ? 'Gestiona el personal para los próximos eventos y contratos'
                            : 'Administra tus listas de personal predefinidas para importación rápida'}
                    </p>
                </div>
                    ...
                    <div className="flex bg-white/5 p-1.5 rounded-2xl border border-white/5">
                        <button 
                            ...
                            className={clsx(
                                ...
                                activeTab === 'convocatorias' ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/20" : "text-gray-500 hover:text-white"
                            )}
                        >
                            Próximas Listas
                        </button>
                        <button 
                            ...
                            className={clsx(
                                "px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all",
                                activeTab === 'plantillas' ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/20" : "text-gray-500 hover:text-white"
                            )}
                        >
                            Plantillas
                        </button>
                    ...
                    >
                        ...
                        {loading ? (
                            ...
                            <div className="bg-surface-card border border-white/5 rounded-[3rem] p-20 text-center flex flex-col items-center">
                                ...
                                <h2 className="text-2xl font-black text-white uppercase mb-2">No hay convocatorias pendientes</h2>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                {filteredConvocatorias.map((evento, idx) => (
                                    <motion.div
                                        ...
                                        className="group relative bg-surface-card border border-white/5 hover:border-indigo-500/30 transition-all duration-500 rounded-[2.5rem] p-6 cursor-pointer overflow-hidden overflow-hidden"
                                    >
                    ...
                    >
                        <div className="flex flex-col sm:flex-row items-center gap-4">
                            ...
                            {isAdminOrDirector && (
                                <Button 
                                    ...
                                    className="h-14 px-8 rounded-2xl shadow-xl shadow-indigo-600/20 shrink-0 w-full sm:w-auto"
                                >
                        ...
                        {loading ? (
                            ...
                            <div className="bg-surface-card border border-white/5 rounded-[3rem] p-20 text-center flex flex-col items-center">
                                ...
                                <h2 className="text-2xl font-black text-white uppercase mb-2">No hay plantillas</h2>
                                <p className="text-gray-500 font-bold uppercase tracking-widest text-[10px]">Crea plantillas para usarlas como base en tus eventos.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {filteredPlantillas.map((plantilla, idx) => (
                                    <motion.div
                                        key={plantilla.id_formacion}
                                        initial={{ opacity: 0, y: 20 }}
                                        animate={{ opacity: 1, y: 0 }}
                                        transition={{ delay: idx * 0.05 }}
                                        className="group relative bg-surface-card border border-white/5 hover:border-indigo-500/30 transition-all duration-500 rounded-[2.5rem] p-8"
                                    >
                                        <div className="flex justify-between items-start mb-6">
                                            <h3 className="text-2xl font-black text-white uppercase tracking-tight line-clamp-1 pr-10">
                                                {plantilla.nombre}
                                            </h3>
                                            <div className="flex gap-2">
                                                <button 
                                                    ...
                                                    className="p-2 hover:bg-white/5 rounded-lg text-gray-500 hover:text-indigo-400 transition-colors"
                                                >
                                                    <Edit className="w-4 h-4" />
                                                </button>
                                                <button 
                                                    onClick={() => handleDeletePlantilla(plantilla.id_formacion)}
                                        ...
                                        <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-6 border-b border-white/5 pb-4">
                                            {plantilla.miembros_count} Miembros registrados
                                        </p>

                                        <div className="flex flex-wrap gap-2">
                                            {plantilla.resumen_instrumentos && Object.entries(plantilla.resumen_instrumentos).map(([inst, count]) => (
                                                <div key={inst} className="px-3 py-1.5 bg-white/5 rounded-xl border border-white/5 flex items-center gap-2">
                                                    <span className="text-[10px] font-black text-indigo-400">{count}</span>
                                                    <span className="text-[9px] font-bold text-gray-500 uppercase tracking-tighter">{inst}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </motion.div>
